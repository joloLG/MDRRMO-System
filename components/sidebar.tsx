...
export function Sidebar({ onAdminViewChange, currentAdminView, unreadFeedbackCount }: SidebarProps) {
  ...
  // Handles clicks for external links (opens in new tab)
  const handleLinkClick = (path: string) => {
    window.open(path, '_blank');
    setIsOpen(false);
  };

  // Handles clicks for internal admin dashboard views
  const handleAdminViewClick = (view: AdminViewType | string) => {
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      >
        ...
        <nav className="flex flex-col gap-2 mt-4">
          {/* Original navigation items (open in new tab) */}
          ...
          <Button variant="ghost" className="justify-start" onClick={() => handleLinkClick("/admin/report-history")}>
            <History className="mr-2 h-4 w-4" /> View History of Report
          </Button>
              <Button
                variant="ghost"
                className={`justify-start ${currentAdminView?.includes('mdrrmo-info') ? 'bg-blue-100 text-blue-800' : ''}`}
                onClick={() => handleAdminViewClick('/admin/mdrrmo-info')}
              >
                <Info className="mr-2 h-4 w-4" /> Edit MDRRMO Info
              </Button>
              <Button
                variant="ghost"
                className={`justify-start ${currentAdminView?.includes('hotlines') ? 'bg-blue-100 text-blue-800' : ''}`}
                onClick={() => handleAdminViewClick('/admin/hotlines')}
              >
                <Phone className="mr-2 h-4 w-4" /> Edit Hotlines
              </Button>
              <Button
                variant="ghost"
                className={`justify-start relative ${currentAdminView?.includes('feedback') ? 'bg-blue-100 text-blue-800' : ''}`}
                onClick={() => handleAdminViewClick('/admin/feedback')}
              >
